...
include("connect.php");
mysql_query("delete from reg where id='$aa'");
$query2="select * from reg";
			 ...
			 $rid2=mysql_query($query2);
				$pagesize=3;
					...
					if(ExecPaging($query2,$parameter,$pagesize))
						$rid=ExecPaging($query2,$parameter,$pagesize,"deleteuser.php");
					else
						$NoMake=1;
	...
	echo"<tr><th>".$rid[$i]['id']."</th><th>".$rid[$i]['first_name']."</th><th>".$rid[$i]['last_name']."</th><th>".$rid[$i]['email_id']."</th><th>"
	.$rid[$i]['password']."</th><th>".$rid[$i]['gender']."</th><th>".$rid[$i]['address']."</th><th>".$rid[$i]['city']."</th><th>"
	.$rid[$i]['ph_no']."</th><th>" ?> <img src="<?php echo $path ; ?>" height="100" width="100"  /><?php echo "</th><th><a href=deleteuser2.php?a="
	.$rid[$i]['id'].">Delete</a></th></tr>";
}
}
echo"</table>";
