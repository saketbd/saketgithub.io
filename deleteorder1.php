...
include("connect.php");
$aa=$_REQUEST['a'];		
mysql_query("delete from orde where id='$aa'");
$query2="select * from orde";
		$rid=0;	
			 $rid2=mysql_query($query2);
				$pagesize=3;
					...
					if(ExecPaging($query2,$parameter,$pagesize))
						$rid=ExecPaging($query2,$parameter,$pagesize,"deleteorder1.php");
					else
						$NoMake=1;
					...
					<th>d.o.b</th><th>Compny name</th>
					<th>model no.</th><th>Price</th>
					<th>Total</th><th>Item</th><th>Email</th><th>image</th><th>Action</th></tr>";
					for($i=0;$i<$num;$i++)
      				{
	$path="upload/".$rid[$i]['image'];
	echo"<tr><th>".$rid[$i]['id']."</th><th>".$rid[$i]['fname']."</th><th>".$rid[$i]['phone']."</th><th>".$rid[$i]['gender']."</th><th>".$rid[$i]['da'].
	"</th><th>".$rid[$i]['cname']."</th><th>".$rid[$i]['mno']."</th><th>".$rid[$i]['price']."</th><th>"
	.$rid[$i]['total']."</th><th>".$rid[$i]['item']."</th><th>"
	.$rid[$i]['email']."</th><th>" ?><img src="<?php echo $path ; ?>" height="100" width="100"  /><?php echo "</th><th>
	<a href=deleteorder1.php?a=".$rid[$i]['id'].">Delete</a></th></tr>";
}
}
echo"</table>";
