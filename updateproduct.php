...
$query2="select * from insrtprdut";
			 ...
			 $rid2=mysql_query($query2);
				$pagesize=3;
					...
					if(ExecPaging($query2,$parameter,$pagesize))
						$rid=ExecPaging($query2,$parameter,$pagesize,"insertproduct.php");
					else
						$NoMake=1;
					...
					<th>features</th>
					<th>Colour</th><th>Image</th><th colspan=2>Action</th></tr>";
					for($i=0;$i<$num;$i++)
      				{
	...
	<th><a href=deleteproduct.php?a=".$rid[$i]['id'].">Delete</a></th>
	<th><a href=Updateproduct.php?a=".$rid[$i]['id'].">Update</a></th>
	</tr>";
}
}
echo"</table>";
	...
	include("connect.php");
$aa=$_REQUEST['a'];
$se=mysql_query("select * from insrtprdut where id='$aa'");
if($sa=mysql_fetch_array($se))
{
	?>
	<h1 class="l1">UPDATE MOBILE</h1>
<form name="f" action="updateproduct1.php" method="post" onSubmit="return val()" enctype="multipart/form-data">
<table class="l2">
<tr><td>Company Name</td><td><input class="l3" type="text" name="t1" value="<?php echo $sa['compny_name'] ; ?>" /></td></tr>
<tr><td>Model No.</td><td><input class="l3" type="text" disabled=disableds name="t2" value="<?php echo $sa['modl_no'] ; ?>" />
<input class="l3" type="hidden" name="t2" value="<?php echo $sa['modl_no'] ; ?>" /></td></tr>
<tr><td>Price</td><td><input class="l3" type="text" name="t3" value="<?php echo $sa['price'] ; ?>" /></td></tr>
<tr><td>Features</td><td><textarea class="l3" name="t4"><?php echo $sa['feturs'] ; ?></textarea></td></tr>
<tr><td>Colour</td><td><input class="l3" type="text" name="t5" value="<?php echo $sa['color'] ; ?>" /></td></tr>
